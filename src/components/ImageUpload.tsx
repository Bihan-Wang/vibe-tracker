...
export default function ImageUpload({ onAnalysisComplete, onUploadComplete }: ImageUploadProps) {
    ...
    try {
      ...
      if (analyzeResult.success && analyzeResult.analysis) {
        ...
        addEntry({
          date: new Date(),
          type: 'image',
          content: `图片记录: ${uploadResult.description || '用户上传的图片'}`,
          mood: analysis.mood,
          moodScore: analysis.moodScore,
          tags: analysis.tags || [],
          aiInsights: analysis.insights || `图片情绪分析: ${analysis.mood}`,
          imageUrl: uploadResult.imageUrl,
        });
  ...
  }, [selectedFile, addEntry, onAnalysisComplete, onUploadComplete]);
        ...
        >
          <div className="flex flex-col items-center justify-center gap-4">
            <div className="w-20 h-20 rounded-full bg-pink-100 dark:bg-pink-900 flex items-center justify-center">
              <Upload className="w-10 h-10 text-pink-600 dark:text-pink-300" />
            </div>
            <div>
            ...
            <button
              className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-lg font-medium hover:from-pink-600 hover:to-purple-600 transition shadow-lg"
              onClick={(e) => {
                e.stopPropagation();
                handleBrowseClick();
              }}
              ...
              {isUploading ? (
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-lg font-medium hover:from-pink-600 hover:to-purple-600 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    上传并分析
                  </button>
                </div>
